...
useEffect(() => {
  ...
  // ============ visual swipe effect (Draggable) ============
  useEffect(() => {
    const slider = heroRef.current;
    if (!slider || !contentRef.current || !bgRef.current) return;

    let draggableInstance = null;

    try {
      // ограничение смещения в пикселях — если потребуется, увеличь/уменьши
      const MAX_DRAG = 260;

      draggableInstance = Draggable.create(slider, {
        type: "x",
        trigger: slider,
        bounds: { minX: -MAX_DRAG, maxX: MAX_DRAG }, // вместо bounds: slider
        edgeResistance: 0.6,
        onDrag: function () {
          // во время перетаскивания двигаем контент и фон (параллакс)
          gsap.set(contentRef.current, { x: this.x });
          // фон с меньшим сдвигом для парралакса
          gsap.set(bgRef.current, { x: this.x * 0.2 });
        },
        onDragEnd: function () {
          const threshold = 80; // пиксели — порог для переключения
          const currentX = this.x;

          if (currentX <= -threshold) {
            // свайп влево -> следующий
            gsap.to([contentRef.current, bgRef.current], { x: 0, duration: 0.25, ease: "power2.out" });
            next();
            ...
            // не дошли до порога — возвращаемся назад
            gsap.to([contentRef.current, bgRef.current], { x: 0, duration: 0.35, ease: "power3.out" });
          }
        },
      })[0];
    } catch (e) {
      // логируем, чтобы в будущем легче отлавливать проблемы
      // eslint-disable-next-line no-console
      console.error("Draggable error:", e);
    }

    return () => {
      if (draggableInstance) draggableInstance.kill();
    };
    // не кладём next/prev в зависимости чтобы не пересоздавать Draggable каждый раз
  ...
  useEffect(() => {
    ...
    gsap.to(bgRef.current, {
      ...
      duration: 0.35,
      onComplete: () => {
        if (bgRef.current) bgRef.current.style.backgroundImage = `url(${activeSlide.bg})`;
  ...
  useEffect(() => {
    ...
    targets.forEach((el) => {
      try {
        ...
        gsap.set(s.chars, { opacity: 0, y: 18 });
        splits.push(s);
      } catch (e) {
        // eslint-disable-next-line no-console
    ...
    splits.forEach((s, i) => {
      tl.to(
        ...
        i === 0 ? 0 : "-=0.25" // частичное перекрытие между строками
      );
    });
